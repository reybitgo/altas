...
class MemberController
{
    ...
    public function reactivate(): void
    {
        ...
        if ($capStatus['cap_status'] !== 'capped') {
            flash('info', 'Your account is not currently capped.');
    ...
    public function doReactivate(): void
    {
        ...
        $userId = Auth::id();
        $result = Reactivation::requestReactivation($userId);

        if ($result['ok']) {
            flash('success', $result['message'] ?? 'Reactivation request submitted.');
        } else {
