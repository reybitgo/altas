...
// ── v2: Conditionally load DailyFixedIncome (Phase 3) ─────────────────────────
$dfiAvailable = false;
...
if (file_exists($dfiPath)) {
    ...
    // Stub: prevents fatal error if Phase 3 not yet deployed
...
try {
    ...
    // ── 6. v2: Expire capped members who missed reactivation window ───────────
    if ($capEngineAvailable) {
        $expired = CapEngine::expireOldCappedUsers();
        if ($expired > 0) {
    ...
    // ── 7. v2: Daily Fixed Income payout ──────────────────────────────────────
    ...
    if ($dfiAvailable) {
        ...
        log_info('DFI payout: Stub active — Phase 3 not yet deployed (core/DailyFixedIncome.php missing).', $logFile);
    }
