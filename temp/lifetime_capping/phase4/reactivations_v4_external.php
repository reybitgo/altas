        ...
        <table class="table table-hover mb-0" style="font-size:.85rem;">
          ...
          <tbody>
            <?php if (empty($result['data'])): ?>
              <tr>
                <td colspan="8" class="text-center py-5 text-muted">No reactivation requests found.</td>
              </tr>
                  ...
                  <?php if ($row['status'] === 'pending'): ?>
                    <div class="d-flex gap-1 flex-wrap">
                      <button class="btn btn-sm btn-success"
                        onclick="reactivationAction('confirm',<?= $row['id'] ?>,'<?= e($row['username']) ?>','<?= fmt_money($row['amount_paid']) ?>','<?= $methodLabel ?>')">
                        ✓ Confirm
                      </button>
                      <button class="btn btn-sm btn-danger"
                        onclick="reactivationAction('reject',<?= $row['id'] ?>,'<?= e($row['username']) ?>','<?= fmt_money($row['amount_paid']) ?>','<?= $methodLabel ?>')">
                        ✕ Reject
                      </button>
                    </div>
                  <?php else: ?>
...
<script>
  function reactivationAction(action, id, user, amount, method) {
    const configs = {
      confirm: {
        title: '✓ Confirm Reactivation',
        desc: `Confirm you've received <strong>${amount}</strong> from <strong>@${user}</strong> via <strong>${method}</strong>.<br>This will reset the member's cap state to active.`,
