...
--  Adds: Lifetime Income Capping, Daily Fixed Income, Reactivation
...
CREATE TABLE packages (
  ...
  -- NEW v2: Lifetime Income Capping
...
CREATE TABLE users (
  ...
  -- NEW v2: Lifetime Income Capping state
  ...
  -- NEW v2: Daily Fixed Income state
...
CREATE TABLE commissions (
  ...
  cap_deduction  DECIMAL(12,2) NOT NULL DEFAULT 0.00,  -- NEW v2: amount blocked by lifetime cap
...
CREATE TABLE ewallet_ledger (
  ...
  ref_type      ENUM('commission','payout','reactivation') NULL,  -- NEW v2: added 'reactivation'
...
-- ─── DAILY FIXED INCOME LOG (NEW v2) ────────────────────────
CREATE TABLE daily_fixed_income_log (
  id                  INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  user_id             INT UNSIGNED  NOT NULL,
  amount              DECIMAL(12,2) NOT NULL,
  day_number          INT UNSIGNED  NOT NULL,  -- which DFI day this was (1-based)
...
ALTER TABLE users          ADD INDEX idx_cap_status    (cap_status, capped_at);           -- NEW v2
ALTER TABLE users          ADD INDEX idx_dfi_active    (dfi_active, dfi_days_used);       -- NEW v2
...
ALTER TABLE reactivations  ADD INDEX idx_user         (user_id, created_at);             -- NEW v2
ALTER TABLE reactivations  ADD INDEX idx_status       (status, created_at);              -- NEW v2
ALTER TABLE daily_fixed_income_log ADD INDEX idx_user_date (user_id, created_at);         -- NEW v2
...
-- Default starter package with v2 capping + DFI settings
...
VALUES (
  ...
  3,
  500.00,
  3.00,      -- lifetime cap = 3 × entry_fee = ₱30,000
  10000.00,  -- reactivation fee
  15,        -- reactivation window (days)
  100.00,    -- daily fixed income per day
  90         -- max DFI days
);
...
INSERT INTO settings (key_name, value) VALUES
  ...
  ('gcash_enabled',     '1'),
  ('maya_enabled',      '1'),
  ('dfi_enabled',       '1');  -- NEW v2: global DFI toggle
