...
{
    $st = td()->prepare("
        SELECT left_count, right_count, left_pv, right_pv,
               paired_pv, paired_pv_today
        FROM users WHERE id = ?
    ");
    $st->execute([$userId]);
    return $st->fetch() ?: [
        'left_count' => 0, 'right_count' => 0,
        'left_pv' => 0.00, 'right_pv' => 0.00,
        'paired_pv' => 0.00, 'paired_pv_today' => 0.00,
    ];
}
...
$sponsorId = register_user([
    ...
    'sponsor_id'         => null,
    'binary_parent_id'   => null,      // true root member — no binary parent
...
test('T4: CD binary placement increments leg counts but does NOT trigger pairing', function () use ($packageId, &$tree) {
    ...
    return [
        ...
        assert_eq($before['paired_pv'],       $afterRight['paired_pv'],  'paired_pv should NOT increase from CD-only placements'),
        assert_eq($before['paired_pv_today'], $afterRight['paired_pv_today'], 'paired_pv_today should NOT increase'),
    ];
});
...
test('T5: Paid placement triggers pairing bonus after CD-built counts', function () use ($packageId, &$tree) {
    ...
    return [
        ...
        assert_eq($before['right_count'],      $afterCd['right_count'],  'CD left placement does NOT affect right_count'),
        assert_gt($before['left_pv'],          $afterCd['left_pv'],      'CD left placement adds PV to left leg'),
        assert_eq($before['paired_pv'],        $afterCd['paired_pv'],    'CD left placement does NOT trigger pair'),
        assert_eq($afterCd['right_count'] + 1, $afterPaid['right_count'], 'Paid right placement increments right_count'),
        assert_gt($afterCd['paired_pv'],       $afterPaid['paired_pv'], 'Paid right placement triggers pairing bonus'),
        assert_gt($afterCd['paired_pv_today'], $afterPaid['paired_pv_today'], 'Paid right placement increments paired_pv_today'),
    ];
});
...
test('T9: Leg counts accurately reflect all placements including CD', function () use ($sponsorId, &$tree) {
    ...
    $pairsLteArms = $info['paired_pv'] <= min($info['left_pv'], $info['right_pv']);
    ...
    return [
        ...
        assert_true($pairsLteArms, "paired_pv ({$info['paired_pv']}) <= min(left_pv,right_pv) (" . min($info['left_pv'], $info['right_pv']) . ")"),
    ];
});
